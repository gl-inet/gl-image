...
static int mptcp_get_int_option(struct mptcp_sock *msk, char __user * optval,
...
static void mptcp_sol_socket_sync_intval(struct mptcp_sock *msk, int optname, char __user * optval,
					   unsigned int optlen)
{
...
static int mptcp_sol_socket_intval(struct mptcp_sock *msk, int optname, char __user * optval,
					   unsigned int optlen)
{
	struct sock *sk = (struct sock *)msk;
	int ret;
	...
	mptcp_sol_socket_sync_intval(msk, optname, optval, optlen);
	return 0;
}

static void mptcp_so_incoming_cpu(struct mptcp_sock *msk, char __user * optval,
					   unsigned int optlen)
{
	struct sock *sk = (struct sock *)msk;
	int val, ret;
	...
	mptcp_sol_socket_sync_intval(msk, SO_INCOMING_CPU, optval, optlen);
}

static int mptcp_setsockopt_sol_socket_tstamp(struct mptcp_sock *msk, int optname, char __user * optval,
					   unsigned int optlen)
{
	struct mptcp_subflow_context *subflow;
	struct sock *sk = (struct sock *)msk;
	...
	mptcp_for_each_subflow(msk, subflow) {
		...
		bool slow = lock_sock_fast(ssk);
		int val, ret;
		mptcp_get_int_option(msk, optval, optlen, &val);
...
*/
	...
	case SO_RCVBUF:
	case SO_RCVBUFFORCE:
		return mptcp_sol_socket_intval(msk, optname, optval, optlen);
	case SO_INCOMING_CPU:
		mptcp_so_incoming_cpu(msk, optval, optlen);
		return 0;
	case SO_TIMESTAMP_OLD:
	case SO_TIMESTAMP_NEW:
	case SO_TIMESTAMPNS_OLD:
	case SO_TIMESTAMPNS_NEW:
		return mptcp_setsockopt_sol_socket_tstamp(msk, optname, optval, optlen);
	}
...
static int mptcp_setsockopt_sol_socket_linger(struct mptcp_sock *msk, char __user * optval,
					      unsigned int optlen)
{
	struct mptcp_subflow_context *subflow;
	struct sock *sk = (struct sock *)msk;
	...
	ret = sock_setsockopt(sk->sk_socket, SOL_SOCKET, SO_LINGER, optval, optlen);
	if (ret)
		return ret;
...
static int mptcp_setsockopt_sol_tcp_congestion(struct mptcp_sock *msk, char __user * optval,
					       unsigned int optlen)
{
	struct mptcp_subflow_context *subflow;
	struct sock *sk = (struct sock *)msk;
	...
	mptcp_for_each_subflow(msk, subflow) {
		...
		lock_sock(ssk);
		err = tcp_set_congestion_control(ssk, name, true, false, cap_net_admin);
		if (err < 0 && ret == 0)
			ret = err;
		subflow->setsockopt_seq = msk->setsockopt_seq;
		release_sock(ssk);
...
static void sync_socket_options(struct mptcp_sock *msk, struct sock *ssk)
{
	...
	if (inet_csk(sk)->icsk_ca_ops != inet_csk(ssk)->icsk_ca_ops)
		tcp_set_congestion_control(ssk, msk->ca_name, false, false, true);
}
