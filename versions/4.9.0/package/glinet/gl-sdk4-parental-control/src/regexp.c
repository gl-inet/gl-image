...
static re *compile(char *regexp)
{
    ...
    for (tail = &head; *regexp != '\0' && err_flag == 0; regexp++) {    // cppcheck-suppress nullPointerRedundantCheck
...
{
    int ret;
    re *regexp = compile(reg);  // cppcheck-suppress nullPointerRedundantCheck
...
static void TEST_reg_func(char *reg, char *str, int ret)
{

    if (ret != regexp_match(reg, str)) {    // cppcheck-suppress nullPointerRedundantCheck
        if (reg)                            // cppcheck-suppress nullPointerRedundantCheck
            printk("reg = %s,", reg);
        else
            printk("reg = null");
...
static void __maybe_unused TEST_regexp(void)
{
