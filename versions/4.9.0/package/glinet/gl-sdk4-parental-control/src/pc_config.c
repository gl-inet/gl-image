...
DEFINE_RWLOCK(monitor_dev_lock);
LIST_HEAD(monitor_dev_list);
...
{
    cJSON *aouobj = NULL, *srcobj = NULL, *logleve = NULL;
    ...
    logleve = cJSON_GetObjectItem(data_obj, "log_level");
    if (!logleve)
        PC_LOG_LEVEL = 2;
    else
        PC_LOG_LEVEL = logleve->valueint;

    PC_INFO("set parental_control log level %d\n", PC_LOG_LEVEL);
    strncpy(pc_src_dev, srcobj->valuestring, MAX_SRC_DEVNAME_SIZE - 1);
    if (0 == strlen(pc_src_dev))
        return 0;

    strncpy(nstr, pc_src_dev, sizeof(nstr) - 1);
    ptr = nstr;
    while (ptr) {
        struct monitor_dev *dev;

        item = strsep(&ptr, " ");
        dev = kzalloc(sizeof(struct monitor_dev), GFP_ATOMIC);
        if (!dev) {
            PC_ERROR("[%s %d] OOM.\n", __FUNCTION__, __LINE__);
            return 0;
        }

        strncpy(dev->ifname, item, IFNAMSIZ);
        dev->ifidex = get_ifindex_by_ifname(item);
        write_lock_bh(&monitor_dev_lock);
        list_add_rcu(&dev->dev_head, &monitor_dev_list);
        write_unlock_bh(&monitor_dev_lock);
    }
...
static int pc_config_handle(char *config, unsigned int len)
{
...
#else
    g_pc_dev.c = class_create(PC_DEV_NAME);
#endif
