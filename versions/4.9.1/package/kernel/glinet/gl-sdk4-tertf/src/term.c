...
#define TERM_CHANGE_EVENT_INTERVAL  (20 * HZ)
...
void term_update(const u8 *mac, __be32 addr, unsigned int rx, unsigned int tx, bool alive)
{
    ...
    if (unlikely(addr != term->ip && addr != 0)) {
        ...
        if (!term->last_change_event ||
                time_after(jiffies, term->last_change_event + TERM_CHANGE_EVENT_INTERVAL)) {
            term->last_change_event = jiffies;
...
static bool term_ipv6_dev_enabled(const struct net_device *dev)
{
    ...
    return idev && !idev->cnf.disable_ipv6;
}

static bool term_ipv6_invalid_saddr(const struct in6_addr *addr)
{
    int i;

    /* Do not emit keepalive NS with unspecified or loopback source address. */
    for (i = 0; i < 15; i++) {
        if (addr->s6_addr[i])
            return false;
    }
...
int send_ns(struct net_device *dev, const struct in6_addr *target_addr)
{
    ...
    if (term_ipv6_invalid_saddr(&sip))
        return err;
