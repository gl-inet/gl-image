...
static void subnet_hlist_del(struct subnet *net)
{
    if (!hlist_unhashed(&net->hlist)) {
        hlist_del_init_rcu(&net->hlist);
        net->ifindex = -1;
    }
}
...
static bool subnet_matches_dev(struct subnet *net, struct net_device *dev)
{
    return !strcmp(net->ifname, dev->name) ||
        (net->ifindex >= 0 && net->ifindex == dev->ifindex);
}

static void subnet_sync_ifname(struct subnet *net, struct net_device *dev)
{
    strncpy(net->ifname, dev->name, sizeof(net->ifname) - 1);
    net->ifname[sizeof(net->ifname) - 1] = '\0';
}

void del_subnet_dev(struct net_device *dev)
{
    struct subnet *net = NULL;

    spin_lock_bh(&lock);
    list_for_each_entry(net, &subnets, list) {
        if (subnet_matches_dev(net, dev)) {
            if (net->ifindex >= 0 && net->ifindex == dev->ifindex)
                subnet_sync_ifname(net, dev);
            subnet_hlist_del(net);
