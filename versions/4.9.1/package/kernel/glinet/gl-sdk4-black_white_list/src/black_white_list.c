...
#define CUSTOM_ISOLATE_IFACE_MAX_LEN 512
#define CUSTOM_ISOLATE_IFINDEX_MAX   64
...
static char gl_custom_isolate_ifaces_str[CUSTOM_ISOLATE_IFACE_MAX_LEN] = {0};
static int gl_custom_isolate_ifindices[CUSTOM_ISOLATE_IFINDEX_MAX];
static int gl_custom_isolate_ifcount;

static DEFINE_SPINLOCK(lock);
static DEFINE_SPINLOCK(net_secure_lock);
static DEFINE_SPINLOCK(mesh_iface_lock);
static DEFINE_SPINLOCK(custom_isolate_lock);
static DEFINE_SPINLOCK(ifidx_lock);

static void clr_isolate_proc_state(void);
...
static void __update_custom_isolate_ifindex(void)
{
    char tmp[CUSTOM_ISOLATE_IFACE_MAX_LEN];
    int new_indices[CUSTOM_ISOLATE_IFINDEX_MAX];
    int new_count = 0;
    char *cursor;
    char *token;

    spin_lock_bh(&custom_isolate_lock);
    strncpy(tmp, gl_custom_isolate_ifaces_str, sizeof(tmp) - 1);
    tmp[sizeof(tmp) - 1] = '\0';
    spin_unlock_bh(&custom_isolate_lock);
    ...
    while ((token = strsep(&cursor, ",")) != NULL) {
        ...
        if (!*token)
            continue;
        if (new_count >= CUSTOM_ISOLATE_IFINDEX_MAX)
            break;
    ...
    spin_lock_bh(&custom_isolate_lock);
    memcpy(gl_custom_isolate_ifindices, new_indices, sizeof(int) * new_count);
    smp_wmb();
    WRITE_ONCE(gl_custom_isolate_ifcount, new_count);
    spin_unlock_bh(&custom_isolate_lock);

    BW_DEBUG("[custom_isolate] update ifindex: bridges=\"%s\", cached %d bridges\n",
             tmp, new_count);
}

static void update_custom_isolate_ifindex(void)
{
    rtnl_lock();
    __update_custom_isolate_ifindex();
    rtnl_unlock();
}
...
static void clr_isolate_proc_state(void)
{
    ...
    spin_lock_bh(&custom_isolate_lock);
    gl_custom_isolate_ifaces_str[0] = '\0';
    spin_unlock_bh(&custom_isolate_lock);
    update_custom_isolate_ifindex();

    spin_lock_bh(&mesh_iface_lock);
    gl_mesh_ifaces_str[0] = '\0';
    spin_unlock_bh(&mesh_iface_lock);
    update_mesh_ifindex();
}

static int custom_isolate_show(struct seq_file *s, void *v)
{
    spin_lock_bh(&custom_isolate_lock);
    seq_printf(s, "%s\n", gl_custom_isolate_ifaces_str);
    spin_unlock_bh(&custom_isolate_lock);

    return 0;
}

static int custom_isolate_open(struct inode *inode, struct file *file)
{
    return single_open(file, custom_isolate_show, NULL);
}

static ssize_t custom_isolate_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
    char tmp[CUSTOM_ISOLATE_IFACE_MAX_LEN];
    int len;
    ...
    spin_lock_bh(&custom_isolate_lock);
    strncpy(gl_custom_isolate_ifaces_str, tmp, CUSTOM_ISOLATE_IFACE_MAX_LEN - 1);
    gl_custom_isolate_ifaces_str[CUSTOM_ISOLATE_IFACE_MAX_LEN - 1] = '\0';
    spin_unlock_bh(&custom_isolate_lock);

    update_custom_isolate_ifindex();

    BW_DEBUG("[custom_isolate] set to: \"%s\", ifcount: %d\n",
             gl_custom_isolate_ifaces_str, READ_ONCE(gl_custom_isolate_ifcount));
...
static const struct file_operations gl_mesh_iface_ops = {
    .owner      = THIS_MODULE,
...
static const struct file_operations gl_custom_isolate_ops = {
    .owner      = THIS_MODULE,
    .open       = custom_isolate_open,
    .read       = seq_read,
    .write      = custom_isolate_write,
    .llseek     = seq_lseek,
    .release    = single_release,
};
#else
...
static const struct proc_ops gl_mesh_iface_ops = {
    .proc_flags      = PROC_ENTRY_PERMANENT,
...
static const struct proc_ops gl_custom_isolate_ops = {
    .proc_flags      = PROC_ENTRY_PERMANENT,
    .proc_open       = custom_isolate_open,
    .proc_read       = seq_read,
    .proc_write      = custom_isolate_write,
    .proc_lseek      = seq_lseek,
    .proc_release    = single_release,
};
#endif
...
static void black_white_list_proc_init(struct proc_dir_entry *proc)
{
    ...
    proc_create("custom_isolate", 0644, proc, &gl_custom_isolate_ops);
    hlist_init();
}
...
static bool is_custom_isolate_ifindex(int ifindex)
{
    int i, cnt;

    cnt = READ_ONCE(gl_custom_isolate_ifcount);
    if (cnt == 0)
        return false;

    smp_rmb();
    for (i = 0; i < cnt; i++) {
        if (READ_ONCE(gl_custom_isolate_ifindices[i]) == ifindex)
            return true;
    }
    return false;
}
...
#endif
{
    ...
    if (is_custom_isolate_ifindex(br_dev->ifindex))
        goto drop;
