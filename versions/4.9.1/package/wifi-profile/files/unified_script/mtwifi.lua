...
local function mtwifi_vap_down(dev, vif)
...
function mtkwifi_vap_wifi_updown(uci, cfgs, diff, device, uci_dev)
    ...
    for k,v in pairs(diff) do
        if k == "ApEnable" then
            for i=1, #vifidx  do
                local uci_vif = mtkdat.get_uci_vif_by_vif_name(uci, vifext..tostring(tonumber(i)-1))
                if token(cfgs[k], i) ~= token(diff[k][2], i) then
                    if token(diff[k][2], i) == "0" then
                        if (vifext..tostring(tonumber(i)-1)) == device.main_ifname then
                            mtwifi_main_dev_down(device, vifext..tostring(tonumber(i)-1))
                        end
                    elseif token(diff[k][2], i) == "1" then
                        add_vif_into_net(vifext..tostring(tonumber(i)-1), uci_vif.network)
                    end
                end
            end
        elseif k == "ApCliEnable" then
            for i=1, 1 do
                local uci_vif = mtkdat.get_uci_vif_by_vif_name(uci, vifapcli..tostring(tonumber(i)-1))
                if token(cfgs[k], i) ~= token(diff[k][2], i) then
                    if token(diff[k][2], i) == "0" then
                        mtwifi_apcli_down(device, vifapcli..tostring(tonumber(i)-1))
                    elseif token(diff[k][2], i) == "1" then
        ...
        if k == "MacAddress" or string.match(k, "MacAddress[0-9]+") then
            local idx
            if k == "MacAddress" then
                idx = 0
            else
                idx = tonumber(string.match(k, "^MacAddress(%d+)$"))
            end
            local uci_vif = mtkdat.get_uci_vif_by_vif_name(uci, vifext..tostring(idx))
            hostapd_disable_vif(vifext..tostring(idx))
            hostapd_setup_vif(uci_dev, uci_vif)
            if (uci_vif.disabled == nil or uci_vif.disabled == '0') then
                hostapd_enable_vif(device.main_ifname, uci_vif)
            end
            nixio.syslog("debug", "mtwifi, diff_wifi_vif_addr vifname: "..vifext..tostring(idx).." mac_addr: "..tostring(diff[k][2]))
        end
    end
end
