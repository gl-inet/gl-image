...
function __set_wifi_apcli_security(cfgs, diff, device, devname)
    ...
    for i = 1, num do
        ...
        if changed then
            ...
            vif.ApcliMacAddress = diff["ApcliMacAddress"] and diff["ApcliMacAddress"][2] or cfgs["ApcliMacAddress"]
        ...
        if vif.ApCliEnable == "1" then
            ...
            commands = commands .."\n".. string.format([[
                ...
                ifconfig %s hw ether %s;]],
                    vif.vifname, vif.vifname, vif.ApcliMacAddress)
            commands = commands.."\n"..string.format([[
                ifconfig %s up;]],
                    vif.vifname)
            ...
            elseif vif.AuthMode == "WPAPSK" then
                ...
                vif.vifname, vif.vifname, vif.vifname,
                get_hostapd_obj("WPA2PSK", vif.EncrypType).rsn_pairwise)
            elseif vif.AuthMode == "WPA2" or vif.AuthMode == "WPA1WPA2" then
                commands = string.format([[
                    wpa_cli -i %s set_network 0 proto WPA2;
                    wpa_cli -i %s set_network 0 key_mgmt WPA-EAP;
                    wpa_cli -i %s set_network 0 pairwise %s;]],
                vif.vifname, vif.vifname, vif.vifname,
                get_hostapd_obj("WPA2PSK", vif.EncrypType).rsn_pairwise)
            else
...
function __set_wifi_down(cfgs, diff, device, devname)
    local vifidx = cfgs.AuthMode and cfgs.AuthMode:split(";") or {}
    local vifname = device.main_ifname
    local vifext = device.ext_ifname
    local vifapcli = device.apcli_ifname
...
-- 接口是否已启动
local function interface_is_up(name)
    ...
    if file then
        ...
        if state == "up" or state == "unknown" then
            return true  -- "up" 或 "unknown" 表示接口已启动
        end
    end

    return false  -- 如果文件不存在或读取失败，默认返回 false
end

function __set_wifi_vif_addr(cfgs, diff, device, devname)
    local vifname = device.main_ifname
    ...
    for i = 0, 15 do
        ...
        local commands = ""
        if i == 0 then
            mac_addr = diff["MacAddress"] and tostring(diff["MacAddress"][2]) or ""
        else
            mac_addr = diff["MacAddress"..tostring(i)] and tostring(diff["MacAddress"..tostring(i)][2]) or ""
        end
        local vif_name = vifname:match("^(%a+)%d*$")..tostring(i)
        if mac_addr ~= nil and mac_addr ~= ""then
            commands = commands.."\n"..string.format([[
            ifconfig %s down;
            ifconfig %s hw ether %s;]],
                vif_name, vif_name, mac_addr)
            nixio.syslog("debug", "mtwifi, __set_wifi_vif_addr vifname: "..vif_name.." mac_addr: "..mac_addr)
            if interface_is_up(vif_name) then
                commands = commands.."\n"..string.format([[
                ifconfig %s up;]],
                    vif_name)
            end
	        nixio.syslog("debug", "mtwifi, __set_wifi_vif_addr commands: "..commands)
            debug_info_write(devname, commands)
        end
    end
end
...
function do_quick_settings(devname, cfgs, diff)
    ...
    __set_wifi_vif_addr(cfgs, diff, device, devname)

    __set_wifi_down(cfgs, diff, device, devname)
