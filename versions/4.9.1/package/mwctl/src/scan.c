...
#define SCAN_OPTIONS "[type=<full|partial|offch|overlap>] [clear] [psc=<1|0>(valid for 6G)] [ssid=<ssid>(valid for full scan)]\n"\
	...
	"[ch_num=<num>(valid for partial)] [dump] [dump=<bss_start_idx>] [dumpall]\n"

#define MAX_ACS_PARAM_LEN 64
#define MAX_SCAN_PARAM_LEN 64
#define MAX_LEN_OF_SSID 32
#define MAX_SCAN_DUMP_LEN 4096
#define MAX_SCAN_DUMPALL_LEN 24576
...
int scan_dumpall_callback(struct nl_msg *msg, void *cb)
{
    struct nlattr *tb[NL80211_ATTR_MAX + 1];
    struct nlattr *vndr_tb[MTK_NL80211_VENDOR_ATTR_SCAN_MAX + 1];
    struct genlmsghdr *gnlh = nlmsg_data(nlmsg_hdr(msg));
    ...
    if (!tb[NL80211_ATTR_VENDOR_DATA]) {
        printf("no any scan dumpall result from driver\n");
        return -EINVAL;
    }
    ...
    if (!vndr_tb[MTK_NL80211_VENDOR_ATTR_GET_SCAN_RESULT_ALL]) {
        printf("no scan dumpall result attr\n");
        return -EINVAL;
    }

    payload_len = nla_len(vndr_tb[MTK_NL80211_VENDOR_ATTR_GET_SCAN_RESULT_ALL]);
    payload = nla_data(vndr_tb[MTK_NL80211_VENDOR_ATTR_GET_SCAN_RESULT_ALL]);
    if (payload_len > MAX_SCAN_DUMPALL_LEN) {
        printf("scan dumpall payload is too long\n");
        return -EINVAL;
    }
    if (payload_len == 0)
        return 0;
...
int scan_dumpall_attr_put(struct nl_msg *msg)
{
    register_handler(scan_dumpall_callback, NULL);
    if (nla_put_flag(msg, MTK_NL80211_VENDOR_ATTR_SCAN_DUMP_ALL))
        return -EMSGSIZE;
