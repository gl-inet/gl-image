                                   ...
                                   int (*parse_dns)(struct sk_buff *, struct dns_query_info *))
{
    ...
    /* Process established TCP connections only. */
...
static struct nf_hook_ops nf_dns_mark_ops[] __read_mostly = {
    {
        ...
        .priority = NF_IP_PRI_CONNTRACK + 1,  /* TCP requires conntrack first. */
    },
    {
        .hook = dns_mark_hook_tcp,
        .pf = NFPROTO_IPV6,
    ...
    },
    {
        ...
        .priority = NF_IP_PRI_FIRST,         /* Run UDP before raw-table CT zone rules. */
    },
    {
        .hook = dns_mark_hook_udp,
        .pf = NFPROTO_IPV6,
