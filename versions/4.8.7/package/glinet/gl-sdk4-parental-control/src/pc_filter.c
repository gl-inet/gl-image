...
uint8_t *parse_tls_server_host_list(uint8_t *data, int len, int *hostlist_len)
{
...
void dpi_https_proto(flow_info_t *flow)
{
...
static int app_in_rule(u_int32_t app, pc_rule_t *rule)
{
    ...
    app = app / MAX_APP_IN_CLASS; // Check the application class after an individual application miss.
    list_for_each_entry(node, &rule->applist, head) {
        if (app == node->app_id)
            return PC_TRUE;
    }
...
static bool is_valid_dns_query_header(unsigned char *dns_header, __be16 *qd)
{
    ...
    if (ntohs(*qd) < 1) // Question count.
        return false;
    if (ntohs(flag) & 0x8000) // QR: 0 is a query and 1 is a response.
        return false;
    if ((ntohs(flag) & 0x7100) >> 11) // Parse standard queries (opcode 0) only.
        return false;
    if ((ntohs(flag) & 0x70)) // Must be zero.
        return false;
...
static void __parse_dns_query(unsigned char *payload, int *len, int *consume, pc_rule_t *rule, int *ret)
{
    ...
    while (*len > 0) {
        ...
        if (token_len > 0 && token_len <= *len) {
            strncpy(token, payload + offset + 1, token_len);
            //printk("use_len:%d token:%s\n", token_len, token);
            update_domain(domain, token, token_len, domain[0] == 0);
            *len -= token_len + 1;
            offset += token_len + 1;
        } else {
            // offset points to the zero byte following the domain name.
            // Add four to skip the fixed query fields: type (2) and class (2).
            *consume += offset + 4 + 1;
            break;
        }
    }
                                    ...
                                    int (*okfn)(struct sk_buff *))
{
    ...
    rule = get_rule_by_mac(smac, &action);
    if (action != PC_POLICY_DROP)
        return NF_ACCEPT;
    ...
    // 24 = UDP header (8) + DNS header (12) + type (2) + class (2).
    ...
    parse_dns_query(dns_payload, dns_payload_len, qd, rule, &drop);
