...
s64 g_offset_time = 0;
char rtmode = 0; // 0: normal mode; 1: passthrough mode.
...
static void add_dev_config(cJSON *data)
{
    ...
    for (i = 0; i < cJSON_GetArraySize(cells_arr); i++) {
        ...
        if (cells) {
            ...
            set_fib_nh(ACTIVE, node); // Ensure the initial route state is active.
...
static void set_base_config(cJSON *data)
{
    ...
    /* Send three heartbeats per detection period; the input unit is milliseconds. */
    g_delay = (unsigned long)(sensitivity->valueint * HZ / 3000);
    /* Calculate the interval remaining after sending three heartbeats. */
