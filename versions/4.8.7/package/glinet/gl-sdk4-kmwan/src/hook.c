...
static void track_node(gl_net_cell_t *node, int tx_flag, bool force)
{
    ...
    if (node->track_mode == PASSIVE && node->state != DEAD) // Probe actively in passive mode only when the interface is dead.
        return;
...
force_send:
    for (i = 0; i < node->track_size; i++) {
        ...
        /* The node must send a packet to check the current network state.
