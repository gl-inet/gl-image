...
static void make_device_submodel(struct device_node *np)
{
    ...
    // Check whether the string contains only 0xff or 0x00 bytes.
    ...
    if (!all_ff) {
        // Find the first 0xff or 0x00 byte in a non-empty value.
        for (i = 0; i < SUBMODEL_LEN; i++) {
            if (p[i] == 0xffffffff || p[i] == 0x00) {
                submodel_len = i ;
                break;
        ...
        // Copy the submodel string up to submodel_len.
        strncpy(gl_hw_info.device_submodel, p, submodel_len);
        gl_hw_info.device_submodel[submodel_len] = '\0'; // Add the terminator.
    }
