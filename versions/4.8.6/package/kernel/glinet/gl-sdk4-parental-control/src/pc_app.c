                              ...
                              port_info_t dport_info, char *host_url, char *request_url, char *dict)
{
    ...
    node->sport = src_port;
    strncpy(node->host_url, host_url, sizeof(node->host_url) - 1);
    strncpy(node->request_url, request_url, sizeof(node->request_url) - 1);
                             ...
                             port_info_t dport_info, char *host_url, char *request_url, char *dict)
{
    ...
    if (node == NULL) {
        ...
        pc_app_write_lock();
        list_add_rcu(&(node->head), &pc_app_head);
        pc_app_write_unlock();
    }
...
static int parse_range_value(char *range_str, range_value_t *range)
{
    ...
    if (range_str[0] == '!') {
        range->not = 1;
        strncpy(pure_range, range_str + 1, sizeof(pure_range) - 1);
    } else {
        range->not = 0;
        strncpy(pure_range, range_str, sizeof(pure_range) - 1);
    }
...
static int parse_app_str(pc_app_t *app, int appid, const char *name, const char *feature)
{
    ...
    while (*p++) {
        ...
        switch (param_num) {
            case PC_PROTO_PARAM_INDEX:
                strncpy(proto_str, begin, min((size_t)(p - begin), sizeof(proto_str) - 1));
                break;
            case PC_SRC_PORT_PARAM_INDEX:
                strncpy(src_port_str, begin, min((size_t)(p - begin), sizeof(src_port_str) - 1));
                break;
            case PC_DST_PORT_PARAM_INDEX:
                strncpy(dst_port_str, begin, min((size_t)(p - begin), sizeof(dst_port_str) - 1));
                break;
            case PC_HOST_URL_PARAM_INDEX:
                strncpy(host_url, begin, min((size_t)(p - begin), sizeof(host_url) - 1));
                break;
            case PC_REQUEST_URL_PARAM_INDEX:
                strncpy(request_url, begin, min((size_t)(p - begin), sizeof(request_url) - 1));
                break;
        }
    ...
    strncpy(dict, begin, min((size_t)(p - begin), sizeof(dict) - 1));
...
static void rcu_free_app(struct rcu_head *head)
{
    pc_app_t *node = container_of(head, pc_app_t, rcu);
    kfree(node);
}
...
void pc_clean_app_feature_list(void)
{
    ...
    list_for_each_entry_safe(node, n, &pc_app_head, head) {
        ...
        call_rcu(&node->rcu, rcu_free_app);
    }
...
int app_proc_show(struct seq_file *s, void *v)
{
    ...
    list_for_each_entry(app, &pc_app_head, head) {
