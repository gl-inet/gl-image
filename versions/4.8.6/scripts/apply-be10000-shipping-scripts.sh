...
# Check every destination before changing any file or following a symlink.
for rel in etc/init.d/boot etc/uci-defaults/99-firewall lib/upgrade/stage2 sbin/sysupgrade; do
...
if (cd "$root" && sha256sum -c "$patchdir/output.sha256" >/dev/null 2>&1); then
    ...
    patch -d "$root" -p1 --batch --fuzz=0 --forward --dry-run -i "$patchdir/100-independent-shipping-scripts.patch"
    patch -d "$root" -p1 --batch --fuzz=0 --forward -i "$patchdir/100-independent-shipping-scripts.patch"
